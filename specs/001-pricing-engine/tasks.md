...
- [x] 8. Conversion and rounding: unit price, total, price per kg, rounded lines, margin absorbing the difference. Tests in all four currencies and for half-up rounding.
- [x] 9. `calculatePrice()` in `lib/pricing/index.ts`: assemble the result with `details`. Test the spec's reference example in CAD and USD.
- [x] 10. Final check: 100% coverage, `npx tsc --noEmit` and `npm run lint` with no errors.
